'use strict'
// CodeBuddy 适配器：读取 ~/.codebuddy/projects/{id}/{sessionId}.jsonl
// 格式：每行一个 JSON 对象，包含 type（message/ai-title 等）字段
...
const PROJECTS_DIR = path.join(os.homedir(), '.codebuddy', 'projects')
...
function parseTranscript(file) {
  ...
  const events = []
  const seenCalls = new Set() // function_call 去重
...
module.exports = { scan, sources, parseFile, id: 'codebuddy', label: 'CodeBuddy', sourceDir: PROJECTS_DIR, sourceFormat: 'jsonl/codebuddy-transcript' }
